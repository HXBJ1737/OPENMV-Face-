...
#-------------Function-------------------
def remove_in_face():
    remove_dirs=os.listdir('face')
#    print(remove_dirs)
    for remove_dir in remove_dirs:
        files=os.listdir('face/'+remove_dir)
        for file in files:
            os.remove('face/'+remove_dir+'/'+file)
        os.rmdir('face/'+remove_dir)
...
#        print(num)
        ...
        pyb.LED(RED_LED_PIN).on()
    img = sensor.snapshot()
    img_drawing_board.draw_string(10,10,'ID:'+str(num),scale=(2),color=(0,255,0),mono_space=False)
    img_drawing_board.b_or(img)
    screen.display(img_drawing_board)
    if num ==1:
        p6.high()
...
#        gray_img=img.to_grayscale()
        ...
        for m,r in enumerate(roi_1):
            ...
            img_drawing_board.draw_string(40,80,'Face Register',scale=(2),color=(255,0,0),mono_space=False)
            img_drawing_board.draw_string(200,80,'Face Identify',scale=(2),color=(255,0,0),mono_space=False)
            img_drawing_board.draw_string(150,20, 'N:'+str(len(os.listdir(rootpath))),scale=(2),color=(0,255,0),mono_space=False)
